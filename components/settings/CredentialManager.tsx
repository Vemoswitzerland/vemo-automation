...
const STATUS_CONFIG = {
  connected: { label: 'Verbunden', dot: 'bg-green-500', text: 'text-green-700', badge: 'bg-green-50 text-green-700 border-green-200' },
  disconnected: { label: 'Nicht konfiguriert', dot: 'bg-gray-400', text: 'text-gray-500', badge: 'bg-gray-100 text-gray-600 border-gray-200' },
  error: { label: 'Fehler', dot: 'bg-red-500', text: 'text-red-600', badge: 'bg-red-50 text-red-700 border-red-200' },
  pending: { label: 'Ausstehend', dot: 'bg-yellow-500', text: 'text-yellow-700', badge: 'bg-yellow-50 text-yellow-700 border-yellow-200' },
}
...
function ConnectorRow({ connector }: { connector: ConnectorWithState }) {
  ...
  return (
    <div className={`border rounded-xl transition-all shadow-sm ${
      expanded ? 'border-gray-300 bg-white' : 'border-gray-200 bg-white'
    }`}>
      {/* Row header */}
      <button
        type="button"
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-gray-500 truncate mt-0.5">{connector.description}</p>
        </div>
          ...
          {connector.fields.length === 0 ? (
            <div className="mt-4">
              ...
              {localStatus !== 'connected' ? (
                ...
                <button onClick={handleDisconnect} className="text-sm text-red-500 hover:text-red-700">
                  Deaktivieren
                </button>
              )}
            </div>
                  ...
                  <div key={field.key}>
                    ...
                    <div className="relative">
                      <input
                        ...
                        className="w-full bg-white border border-gray-300 rounded-lg px-3 py-2 text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:border-vemo-green-500 focus:ring-1 focus:ring-vemo-green-500 transition-colors pr-8"
                      />
                      {field.type === 'password' && (
                        <button
                          type="button"
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="card text-center">
          ...
          <div className="text-xs text-gray-500 mt-1">Nicht konfiguriert</div>
        </div>
        <div className="card text-center">
      ...
      {connected === 0 && (
        <div className="card border-yellow-200 bg-yellow-50">
          <div className="flex gap-3">
            ...
            <div>
              <p className="font-semibold text-yellow-800 text-sm">Erste Schritte</p>
